...
module bus_rx_flow (
  ...
  always_comb begin : blockName
    if (rx_bit_en & ((scl_stable_high_i & bit_counter == 4'h7) | scl_posedge_i)) begin
      rx_bit <= sda_i;
    end else begin
      rx_bit <= '0;
    end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin : aggregate_bits_from_bus
    if (~rst_ni) begin
      rx_data <= '0;
    end else begin
      if (bit_counter_en) begin
        if (rx_done)
          rx_data[7:0] <= {rx_data[6:0], rx_bit};
      end else begin
        rx_data <= '0;
      end
    end
  ...
  always_comb begin : rx_fsm_state
    ...
    unique case (state_q)
      Idle: begin
        state_d = rx_req_byte_i ? ReadByte :
                  rx_req_bit_i  ? ReadBit  : Idle;
      end
      ReadByte: begin
        if (rx_done_o) begin
