...
#(
  ...
  always_ff @( posedge clk_i or negedge rst_ni ) begin : proc_last_byte
    if(~rst_ni) begin
      ...
      if (bus_rx_done_i & bus_rx_req_byte_o)
        last_byte <= bus_rx_data_i;
    end
  end
  ...
  // ACK, T-bit, Parity
  logic ack_done, tbit_done, parity_bit;
  assign ack_done = bus_tx_done_i;
  assign tbit_done = bus_tx_done_i;
  assign parity_bit = ^last_byte;
  ...
  assign is_our_addr_match = target_dyn_address_valid_i ? (target_dyn_address_i == bus_addr_q) :
                             target_sta_address_valid_i ? (target_sta_address_i == bus_addr_q) : 1'b0;
  assign is_rsvd_byte_match = ({bus_addr_q, bus_rnw_q} == `I3C_RSVD_BYTE);
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin : latch_rx_overflow_error
    if (~rst_ni) begin
      ...
      if (state_d == RxPWriteData & ~rx_fifo_wready_i)
        rx_overflow_err_r <= 1'b1;
      else if (target_idle_o | state_d inside {RxFByte, Idle})
        rx_overflow_err_r <= 1'b0;
    end
  end
  ...
  assign rx_fifo_wvalid_o = (state_q == RxPWriteData) & (state_d != RxPWriteData) & ~(parity_err | rx_overflow_err_o);
  // Last RX byte when we leave Private Read loop
  assign rx_last_byte_o = (state_q == TxPReadData) & (state_d inside {RxFByte, Idle});
  ...
  always_comb begin : state_outputs
    ...
    bus_tx_req_byte_o = '0;
    bus_tx_req_bit_o = '0;
    bus_tx_req_value_o = 8'h1;  // Pullup
    tx_host_nack_o = '0;
    rx_fifo_wdata_o = '0;
    ibi_fifo_rready_o = '0;
    bus_addr_d = '0;
    ...
    case (state_q)
      ...
      RxSByte: begin
        bus_rx_req_byte_o = ~bus_start_det_i;
        if (bus_rx_done_i) begin
          bus_addr_valid = 1'b1;
          bus_addr_d = bus_rx_data_i[7:1];
          bus_rnw_d = bus_rx_data_i[0];
  ...
  always_comb begin : state_transitions
    ...
    case (state_q)
      Idle: begin
        if (target_enable_i)
          state_d = target_reset_detect_i ? DoRstAction :
                    ...
                    is_ibi_pending        ? DoIBI :
                    bus_start_det_i       ? RxFByte : Idle;
        end
      RxFByte: begin
        if (bus_rx_done_i) begin
          state_d = CheckFByte;
      ...
      CheckFByte: begin
        if (is_rsvd_byte_match || is_our_addr_match)
          state_d = TxAckFByte;
        else
          state_d = Wait;
      end
      TxAckFByte: begin
        if (ack_done) begin
          if (is_rsvd_byte_match)
              state_d = RxSByte;
          else if (is_our_addr_match && bus_rnw_q)
              state_d = TxPReadData;
          else if (is_our_addr_match && ~bus_rnw_q)
      ...
      TxAckSByte: begin
        if (ack_done) begin
          if (is_our_addr_match && bus_rnw_q)
              state_d = TxPReadData;
          else if (is_our_addr_match && ~bus_rnw_q)
              state_d = RxPWriteData;
          else
    ...
    if (target_reset_detect_i)
      state_d = DoRstAction;
    else if (hdr_exit_detect_i)
      state_d = DoHdrExit;
  end
  ...
  assign target_transmitting_o = (state_q inside {TxAckFByte, TxAckSByte, TxPReadData, TxPReadTbit});
