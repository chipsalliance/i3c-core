...
module bus_tx_flow (
  ...
  input logic abort_i,  // Force release the bus (abort drive request)
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin : update_req_value
    if (~rst_ni) begin
      ...
      if (bit_counter_en) begin
        if (tx_done)
          req_value[7:0] <= {req_value[6:0], 1'b0};
  ...
  always_comb begin : tx_fsm_outputs
    ...
    unique case (state_q)
      ...
      NextTaskDecision: begin
        drive_bit_en = req;
        drive_bit_value = req_byte_i ? req_value[7] :
                          req_bit_i ? req_value[0] : 1'b1;
      end
      default: ;
    endcase
  end
  ...
  always_comb begin : tx_fsm_state
    ...
    unique case (state_q)
      Idle: begin
        if (tx_idle) begin
          state_d = req_byte_i ? DriveByte :
                    req_bit_i ? DriveBit : Idle;
        end
      end
      DriveByte: begin
    ...
    // Allow to abort and go back to Idle if needed, it takes precedence over requesets
    if (abort_i | error) begin
      state_d = Idle;
    end
  end
