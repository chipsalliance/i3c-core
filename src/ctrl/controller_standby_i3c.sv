...
#(
    ...
    parameter int unsigned TtiTxFifoDepthWidth = $clog2(TtiTxFifoDepth + 1),  // FIXME
  ...
  // TTI TX Queue interface
  logic tx_fifo_rvalid;
  logic tx_fifo_rready;
  logic [TtiTxDataWidth-1:0] tx_fifo_rdata;
  logic tx_host_nack;

  // TTI RX Queue interface
  logic rx_fifo_wvalid;
  logic [TtiRxDataWidth-1:0] rx_fifo_wdata;
  logic rx_fifo_wready;
  logic rx_last_byte;
  logic tx_last_byte;

  // TTI IBI Queue interface
  logic ibi_fifo_rvalid;
  logic ibi_fifo_rready;
  logic [TtiIbiDataWidth-1:0] ibi_fifo_rdata;
  logic ibi_last_byte;

  // Bus events notiffications
  ...
  logic [7:0] bcr;
  logic [7:0] dcr;
  logic [6:0] target_sta_address;
  logic target_sta_address_valid;
  logic [6:0] target_dyn_address;
  logic target_dyn_address_valid;
  logic [6:0] target_ibi_address;
  logic target_ibi_address_valid;
  logic [6:0] target_hot_join_address;
  ...
  logic set_newda;
  logic [6:0] newda;
  logic [15:0] get_mwl; // FIXME: `get_*` might suggest that it is a single bit trigger/enable signal
  logic [15:0] get_mrl;
  logic [47:0] get_pid;
  logic [7:0] get_bcr;
  logic [7:0] get_dcr;
  ...
  i3c_target_fsm #(
      ...
      .target_sta_address_i       (target_sta_address),
      .target_sta_address_valid_i (target_sta_address_valid),
      .target_dyn_address_i       (target_dyn_address),
      .target_dyn_address_valid_i (target_dyn_address_valid),
      .target_ibi_address_i       (target_ibi_address),
      .target_ibi_address_valid_i (target_ibi_address_valid),
  ...
  addr_match u_addr_match (
      .clk_i,
      .rst_ni,
      .stby_cr_device_addr_reg_i  (stby_cr_device_addr_reg_i),
      .stby_cr_device_char_reg_i  (stby_cr_device_char_reg_i),
      .stby_cr_device_pid_lo_reg_i(stby_cr_device_pid_lo_reg_i),
      .pid_o                      (pid),
      .bcr_o                      (bcr),
      .dcr_o                      (dcr),
      .target_sta_addr_o          (target_sta_address),
      .target_sta_addr_valid_o    (target_sta_address_valid),
      .target_dyn_addr_o          (target_dyn_address),
      .target_dyn_addr_valid_o    (target_dyn_address_valid),
      .target_ibi_addr_o          (target_ibi_address),
      .target_ibi_addr_valid_o    (target_ibi_address_valid),
      .target_hot_join_addr_o     (target_hot_join_address),
      .daa_unique_response_o      (daa_unique_response)
  );

  descriptor_rx #(
      .TtiRxDescDataWidth(32),
      .TtiRxDataWidth    (8)
  ) u_descriptor_rx (
      .clk_i                     (clk_i),
      .rst_ni                    (rst_ni),
