...
class TTIQueuesTestInterface(HCIBaseTestInterface):
    ...
    async def read_queue_size(self, queue: str):
        ...
        off = {"tti_rx_desc": 0, "tti_tx_desc": 8, "tti_rx": 16, "tti_tx": 24}
    ...
    # of the queues
    async def put_rx_desc(self, data: int = None, timeout: int = 2, units: str = "ms"):
        if not data:
    ...
    async def get_tx_desc(self, timeout: int = 2, units: str = "ms") -> int:
    ...
    async def put_rx_data(self, data: int = None, timeout: int = 2, units: str = "ms"):
        if not data:
            data = randint(1, 2**32 - 1)
