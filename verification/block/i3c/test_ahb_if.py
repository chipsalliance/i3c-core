...
async def setup_dut(dut: SimHandle, clk_period: Tuple[int, str]) -> None:
    ...
    """
    await cocotb.start(Clock(dut.clk_i, *clk_period).start())
    dut.rst_ni.value = 0
...
class AHBFIFOTestInterface:
    ...
    async def register_test_interfaces(self):
        # Clocks & resets
        self.AHBManager.register_clock(self.dut.clk_i).register_reset(self.dut.rst_ni, True)
        self.interconnect.register_clock(self.dut.clk_i).register_reset(self.dut.rst_ni, True)
        self.wrapper.register_clock(self.dut.clk_i).register_reset(self.dut.rst_ni, True)
        # Interconnect setup
