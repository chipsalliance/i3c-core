// Copyright lowRISC contributors (OpenTitan project).
// Licensed under the Apache License, Version 2.0, see LICENSE for details.
// SPDX-License-Identifier: Apache-2.0

// FPV CSR read and write assertions auto-generated by `reggen` containing data structure
// Do Not Edit directly
// TODO: This automation currently only support register without HW write access

`include "caliptra_prim_assert.sv"

`ifndef FPV_ON
  `define REGWEN_PATH tb.dut.u_reg
`else
  `define REGWEN_PATH i2c.u_reg
`endif

// Block: i2c
module i2c_csr_assert_fpv import tlul_pkg::*;
    import top_pkg::*;(
  input clk_i,
  input rst_ni,

  // tile link ports
  input tl_h2d_t h2d,
  input tl_d2h_t d2h
);

`ifdef UVM
  import uvm_pkg::*;
`endif

// Currently FPV csr assertion only support HRO registers.
`ifndef VERILATOR
`ifndef SYNTHESIS

  logic oob_addr_err;

  typedef struct packed {
    logic [TL_DW-1:0] wr_data;
    logic [TL_AW-1:0] addr;
    logic             wr_pending;
    logic             rd_pending;
  } pend_item_t;

  bit disable_sva;

  // mask register to convert byte to bit
  logic [TL_DW-1:0] a_mask_bit;

  assign a_mask_bit[7:0]   = h2d.a_mask[0] ? '1 : '0;
  assign a_mask_bit[15:8]  = h2d.a_mask[1] ? '1 : '0;
  assign a_mask_bit[23:16] = h2d.a_mask[2] ? '1 : '0;
  assign a_mask_bit[31:24] = h2d.a_mask[3] ? '1 : '0;

  // An index used for regwen, access_policy and exp_vals. These arrays all have one element per HRO
  // register, plus an extra dummy entry to represent "non-HRO registers" (avoiding out-of-bounds
  // accesses).
  bit [4:0] hro_idx;
  bit [6:0] normalized_addr;

  `ifdef FPV_ON
    // For FPV, we restrict the pend_trans array by giving its size a smaller upper bound. This
    // reduces the FPV runtime dramatically.
    localparam int PendTransLen = 11;

    // These two assumptions bound h2d.a_source and d2h.d_source for FPV because they are used as
    // indices for the array.
    `CALIPTRA_ASSUME_FPV(TlulSourceA_M, h2d.a_source >=  0 && h2d.a_source < PendTransLen, clk_i, !rst_ni)
    `CALIPTRA_ASSUME_FPV(TlulSourceD_M, d2h.d_source >=  0 && d2h.d_source < PendTransLen, clk_i, !rst_ni)
  `else
    localparam int PendTransLen = 2**TL_AIW;
  `endif

  pend_item_t [PendTransLen-1:0] pend_trans;

  // Indexes of pend_trans have a width that depends on the size of the array. These lines extract
  // just the bottom bits (so that the widths match).
  //
  // When in FPV mode, the TlulSource*_M assumptions above ensure that we aren't dropping any
  // nonzero bits. When not in FPV mode, this is a no-op: the a_source and d_source values are of
  // width TL_AIW and we extract that many bits.
  localparam int PendTransIdxWidth = $clog2(PendTransLen);
  logic [PendTransIdxWidth-1:0] a_source_idx, d_source_idx;

  assign a_source_idx = h2d.a_source[PendTransIdxWidth-1:0];
  assign d_source_idx = d2h.d_source[PendTransIdxWidth-1:0];

  // Map register address with hro_idx in exp_vals array.
  always_comb begin: decode_hro_addr_to_idx
    unique case (pend_trans[d_source_idx].addr)
      4: hro_idx <= 0;
      8: hro_idx <= 1;
      12: hro_idx <= 2;
      16: hro_idx <= 3;
      28: hro_idx <= 4;
      32: hro_idx <= 5;
      36: hro_idx <= 6;
      40: hro_idx <= 7;
      52: hro_idx <= 8;
      60: hro_idx <= 9;
      64: hro_idx <= 10;
      68: hro_idx <= 11;
      72: hro_idx <= 12;
      76: hro_idx <= 13;
      80: hro_idx <= 14;
      84: hro_idx <= 15;
      92: hro_idx <= 16;
      96: hro_idx <= 17;
      100: hro_idx <= 18;
      108: hro_idx <= 19;
      // If the register is not a HRO register, the write data will all update to this default idx.
      default: hro_idx <= 20;
    endcase
  end

  // store internal expected values for HW ReadOnly registers
  logic [TL_DW-1:0] exp_vals[21];

  // Word-align the incoming TLUL a_address to obtain the normalized address.
  assign normalized_addr = {h2d.a_address[6:2], 2'b0};

  // Assign regwen to registers. If the register does not have regwen, it will default to value 1.
  logic [20:0] regwen;
  assign regwen[0] = 1;
  assign regwen[1] = 1;
  assign regwen[2] = 1;
  assign regwen[3] = 1;
  assign regwen[4] = 1;
  assign regwen[5] = 1;
  assign regwen[6] = 1;
  assign regwen[7] = 1;
  assign regwen[8] = 1;
  assign regwen[9] = 1;
  assign regwen[10] = 1;
  assign regwen[11] = 1;
  assign regwen[12] = 1;
  assign regwen[13] = 1;
  assign regwen[14] = 1;
  assign regwen[15] = 1;
  assign regwen[16] = 1;
  assign regwen[17] = 1;
  assign regwen[18] = 1;
  assign regwen[19] = 1;
  assign regwen[20] = 1;

  typedef enum bit {
    FpvDefault,
    FpvRw0c
  } fpv_reg_access_e;
  fpv_reg_access_e access_policy [21];

  // for write HRO registers, store the write data into exp_vals
  always_ff @(negedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
       oob_addr_err <= 1'b0;
       pend_trans <= '0;
       exp_vals[0] <= 'h0;
       access_policy[0] <= FpvDefault;
       exp_vals[1] <= 'h0;
       access_policy[1] <= FpvDefault;
       exp_vals[2] <= 'h0;
       access_policy[2] <= FpvDefault;
       exp_vals[3] <= 'h0;
       access_policy[3] <= FpvDefault;
       exp_vals[4] <= 'h0;
       access_policy[4] <= FpvDefault;
       exp_vals[5] <= 'h0;
       access_policy[5] <= FpvDefault;
       exp_vals[6] <= 'h0;
       access_policy[6] <= FpvDefault;
       exp_vals[7] <= 'h0;
       access_policy[7] <= FpvDefault;
       exp_vals[8] <= 'h0;
       access_policy[8] <= FpvDefault;
       exp_vals[9] <= 'h0;
       access_policy[9] <= FpvDefault;
       exp_vals[10] <= 'h0;
       access_policy[10] <= FpvDefault;
       exp_vals[11] <= 'h0;
       access_policy[11] <= FpvDefault;
       exp_vals[12] <= 'h0;
       access_policy[12] <= FpvDefault;
       exp_vals[13] <= 'h0;
       access_policy[13] <= FpvDefault;
       exp_vals[14] <= 'h0;
       access_policy[14] <= FpvDefault;
       exp_vals[15] <= 'h0;
       access_policy[15] <= FpvDefault;
       exp_vals[16] <= 'h0;
       access_policy[16] <= FpvDefault;
       exp_vals[17] <= 'h0;
       access_policy[17] <= FpvDefault;
       exp_vals[18] <= 'h0;
       access_policy[18] <= FpvDefault;
       exp_vals[19] <= 'h0;
       access_policy[19] <= FpvDefault;
       exp_vals[20] <= 'h0;
       access_policy[20] <= FpvDefault;
    end else begin
      oob_addr_err <= 1'b0;
      if (h2d.a_valid && d2h.a_ready) begin
        if ((normalized_addr inside {[0:112]})
           ) begin
          pend_trans[a_source_idx].addr <= normalized_addr;
          if (h2d.a_opcode inside {PutFullData, PutPartialData}) begin
            pend_trans[a_source_idx].wr_data <= h2d.a_data & a_mask_bit;
            pend_trans[a_source_idx].wr_pending <= 1'b1;
          end else if (h2d.a_opcode == Get) begin
            pend_trans[a_source_idx].rd_pending <= 1'b1;
          end
        end else begin
          oob_addr_err <= 1'b1;
        end
      end
      if (d2h.d_valid) begin
        if (pend_trans[d_source_idx].wr_pending == 1) begin
          if (!d2h.d_error && regwen[hro_idx]) begin
            if (access_policy[hro_idx] == FpvRw0c) begin
              // Assume FpvWr0c policy only has one field that is wr0c.
              exp_vals[hro_idx] <= exp_vals[hro_idx][0] == 0 ? 0 : pend_trans[d_source_idx].wr_data;
            end else begin
              exp_vals[hro_idx] <= pend_trans[d_source_idx].wr_data;
            end
          end
          pend_trans[d_source_idx].wr_pending <= 1'b0;
        end
        if (h2d.d_ready && pend_trans[d_source_idx].rd_pending == 1) begin
          pend_trans[d_source_idx].rd_pending <= 1'b0;
        end
      end
    end
  end

  // for read HRO registers, assert read out values by access policy and exp_vals
  `CALIPTRA_ASSERT(intr_enable_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h4 |->
         d2h.d_error ||
         (d2h.d_data & 'h7fff) == (exp_vals[0] & 'h7fff))

  `CALIPTRA_ASSERT(ctrl_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h10 |->
         d2h.d_error ||
         (d2h.d_data & 'h7) == (exp_vals[3] & 'h7))

  `CALIPTRA_ASSERT(host_fifo_config_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h24 |->
         d2h.d_error ||
         (d2h.d_data & 'hfff0fff) == (exp_vals[6] & 'hfff0fff))

  `CALIPTRA_ASSERT(target_fifo_config_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h28 |->
         d2h.d_error ||
         (d2h.d_data & 'hfff8fff) == (exp_vals[7] & 'hfff8fff))

  `CALIPTRA_ASSERT(ovrd_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h34 |->
         d2h.d_error ||
         (d2h.d_data & 'h7) == (exp_vals[8] & 'h7))

  `CALIPTRA_ASSERT(timing0_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h3c |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[9] & 'hffffffff))

  `CALIPTRA_ASSERT(timing1_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h40 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[10] & 'hffffffff))

  `CALIPTRA_ASSERT(timing2_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h44 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[11] & 'hffffffff))

  `CALIPTRA_ASSERT(timing3_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h48 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[12] & 'hffffffff))

  `CALIPTRA_ASSERT(timing4_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h4c |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[13] & 'hffffffff))

  `CALIPTRA_ASSERT(timeout_ctrl_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h50 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[14] & 'hffffffff))

  `CALIPTRA_ASSERT(target_id_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h54 |->
         d2h.d_error ||
         (d2h.d_data & 'hfffffff) == (exp_vals[15] & 'hfffffff))

  `CALIPTRA_ASSERT(host_timeout_ctrl_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h60 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[17] & 'hffffffff))

  `CALIPTRA_ASSERT(target_timeout_ctrl_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h64 |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[18] & 'hffffffff))

  `CALIPTRA_ASSERT(host_nack_handler_timeout_rd_A, d2h.d_valid && pend_trans[d_source_idx].rd_pending &&
         pend_trans[d_source_idx].addr == 7'h6c |->
         d2h.d_error ||
         (d2h.d_data & 'hffffffff) == (exp_vals[19] & 'hffffffff))


  `CALIPTRA_ASSERT(TlulOOBAddrErr_A, oob_addr_err |-> s_eventually(d2h.d_valid && d2h.d_error))

  `ifdef UVM
    initial forever begin
      bit csr_assert_en;
      uvm_config_db#(bit)::wait_modified(null, "%m", "csr_assert_en");
      if (!uvm_config_db#(bit)::get(null, "%m", "csr_assert_en", csr_assert_en)) begin
        `uvm_fatal("csr_assert", "Can't find csr_assert_en")
      end
      disable_sva = !csr_assert_en;
    end
  `endif

`endif
`endif
endmodule

`undef REGWEN_PATH

